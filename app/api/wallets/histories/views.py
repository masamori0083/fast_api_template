...
router = APIRouter(prefix="/histories", route_class=LoggingRoute)
...
async def get_histories(
    ...
    return GetHistoriesResponse(
        histories=[
            History.model_validate(h)
            for h in await use_case.execute(wallet_id=wallet_id)
        ]
    )


@router.get("/{history_id}", response_model=GetHistoryResponse)
async def get_history(
    ...
    return GetHistoryResponse.model_validate(
        await use_case.execute(wallet_id=wallet_id, history_id=history_id),
    )


@router.post(
    "", response_model=PostHistoryResponse, status_code=status.HTTP_201_CREATED
)
...
@router.put("/{history_id}", response_model=PutHistoryResponse)
...
@router.post("{history_id}/move", response_model=MoveHistoryResponse, description="")
